...
- **GRADE-adapted confidence labeling** is the highest-value transfer: it forces explicit uncertainty quantification at every claim, directly addressing the false-precision bias the AGENTS.md process already warns against.
...
- **Anti-cherry-picking rules** from inclusion/exclusion criteria fill a structural gap: no existing AGENTS.md mechanism prevents agents from silently dropping sources that contradict the emerging narrative.
...
### Current State in the AGENTS.md Process
...
The RAISE framework (Cochrane/Campbell/JBI 2024-25) represents current expert consensus: GenAI in evidence synthesis requires human oversight [PRIMARY]. For AGENTS.md: adversarial verification is not optional overhead but a necessary correction for systematic AI error.
...
| Recommendation | Change to AGENTS.md | Expected Impact | Risk if Wrong |
|---------------|---------------------|----------------|---------------|
| MECIR-style mandatory/desirable quality tiers | Classify AGENTS.md quality standards as mandatory vs. desirable | Enables principled abbreviation for lower-stakes research | Medium: wrong classification could weaken standards |
