...
**Feasibility**: High. Prompt reframing is a zero-cost intervention. The AGENTS.md process already requires counter-searches (INV-G25), but this makes the rationale explicit: it's not just good practice — it's a specific mitigation for a documented AI failure mode.
...
The AGENTS.md process already implements a multi-layered approach (source authority hierarchy + cross-referencing + adversarial verification). The AI failure mode evidence validates this as the correct architecture and suggests specific additions:
...
1. **Research synthesis-specific hallucination rates are unmeasured**: Most empirical data comes from summarization, QA, or biography generation. How hallucination manifests in multi-source research synthesis — the specific task AGENTS.md targets — is under-studied. [Gap severity: Significant]
